...
import type { MarketEntry, InstallConfig, InstallResult, VersionInfo } from '../types/market.types';
import type { InstalledPlugin, PluginRegistry } from './plugin-registry.service';
import { MarketService, createMarketService } from './market.service';
...
export class PluginInstaller {
   ...
   * 卸载插件
   */
  async uninstall(pluginId: string, options?: { removeFiles?: boolean }): Promise<{
    success: boolean;
    error?: string;
  }> {
    ...
    try {
      ...
      // 更新注册表状态
      this.registry.updateStatus(pluginId, 'removing');
   ...
   * 更新插件
   */
  async update(pluginId: string, targetVersion?: string): Promise<InstallResult> {
    try {
      const plugin = this.registry.get(pluginId);
      if (!plugin) {
        return {
   ...
   * 检查所有已安装插件的更新
   */
  async checkAllUpdates(): Promise<Map<string, {
    hasUpdate: boolean;
   ...
   * 批量安装插件
   */
  async installBatch(configs: InstallConfig[]): Promise<InstallResult[]> {
   ...
   * 下载插件到目标路径
   */
  private async downloadPlugin(url: string, targetPath: string): Promise<void> {
    try {
      // 确保目标目录存在
      ...
      throw new PluginInstallerError(
        `下载插件失败：${error instanceof Error ? error.message : '未知错误'}`,
        'DOWNLOAD_FAILED'
      );
    }
  }
   ...
   * 从市场条目提取依赖
   */
  private extractDependencies(marketEntry: MarketEntry): string[] {
    // 从 marketEntry 中提取依赖信息
    // 假设市场条目包含 dependencies 字段
    return (marketEntry as unknown as { dependencies?: string[] }).dependencies || [];
  }

  /**
   * 保存安装配置到本地
   */
  private async saveInstallConfig(plugin: InstalledPlugin): Promise<void> {
   ...
   * 删除本地安装配置
   */
  private async removeInstallConfig(pluginId: string): Promise<void> {
    const configPath = join(this.baseDir, 'installs.json');
