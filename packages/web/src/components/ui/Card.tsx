...
export function Card(props: { title?: string; actions?: ReactNode; children: ReactNode; className?: string; compact?: boolean }): JSX.Element {
  ...
  const headerPaddingClassName = compact ? 'px-3 py-2.5' : 'px-4 py-3';
  const contentPaddingClassName = compact ? 'p-3' : 'p-4';

  return (
    <section className={`rounded-xl border border-slate-200 bg-white shadow-sm ${props.className ?? ''}`.trim()}>
      {props.title ? (
        <div className={`flex items-start justify-between gap-3 border-b border-slate-100 ${headerPaddingClassName}`}>
