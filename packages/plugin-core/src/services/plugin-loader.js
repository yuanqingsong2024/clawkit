...
export class PluginLoader {
     ...
     * 加载插件配置文件
     */
    async loadPluginConfig(pluginDir) {
        ...
        try {
            ...
            if (rawDeps && typeof rawDeps === 'object' && !Array.isArray(rawDeps)) {
                dependencies = Object.fromEntries(Object.entries(rawDeps).filter(([, v]) => typeof v === 'string'));
            }
            const meta = {
                name,
                version,
     ...
     * 注册并加载发现的插件
     */
    async registerDiscoveredPlugins(manager, context) {
        const discovered = await this.discover();
        const results = [];
        for (const plugin of discovered) {
            try {
                // 注册插件
                const registerResult = manager.register(plugin.meta.type, plugin.meta.name, plugin.meta.version, plugin.config, context);
     ...
     */
    async discoverWorkspacePlugins(workspaceRoot) {
        const plugins = [];
        try {
                    ...
                    for (const entry of entries) {
                        if (entry.isDirectory()) {
                            ...
                            try {
                                ...
                                const keywords = (pluginPkg.keywords || []).map(k => k.toLowerCase());
     ...
     * 验证插件路径安全性
     */
    async validatePath(pluginPath, allowedPaths) {
 ...
 * 从配置文件加载插件列表
 */
export async function loadPluginManifest(manifestPath) {
    try {
