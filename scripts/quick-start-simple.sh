...
# clawkit 简化版一键部署脚本
# 用途：使用简化配置快速部署单机模式
# 优先使用 simple.yaml，向后兼容 all-in-one.yaml
...
用法：
  ./scripts/quick-start-simple.sh [选项]

选项：
  -f, --file <path>    manifest 文件路径（默认：自动选择）
...
if [ -z "$MANIFEST_FILE" ]; then
  if MANIFEST_FILE=$(find_manifest_file); then
    log_info "自动选择配置文件：$MANIFEST_FILE"
  else
    log_error "未找到配置文件，请创建 clawkit.yaml 或使用 -f 指定配置文件"
    log_info "提示：可以从 examples/simple.yaml 复制一份作为起点"
    exit 1
  fi
fi
...
if [ "$NO_START" = true ]; then
  ...
  # 设置环境变量
  export CLAWKIT_MANIFEST_PATH="$(node -e "console.log(require('node:path').resolve(process.argv[1]))" "$MANIFEST_FILE")"
...
log_info "  3. 管理项目：node ./packages/cli/dist/index.js project list"
log_info "  4. 查看状态：node ./packages/cli/dist/index.js status"
log_info "  5. 停止服务：kill \$(cat .clawkit/controller.pid) \$(cat .clawkit/worker.pid)"
echo ""
log_success "部署完成！"
